...
PHP_FUNCTION(array_unique)
{
	...
	zend_sort((void *) arTmp, i, sizeof(struct bucketindex), array_bucketindex_swap, php_array_data_compare);
		...
		if (hash->nNumOfElements > 1) {
			if (behavior == INTERSECT_NORMAL) {
				zend_sort((void *) lists[i], hash->nNumOfElements, 
						sizeof(Bucket), zend_hash_bucket_swap, intersect_data_compare_func);
			} else if (behavior & INTERSECT_ASSOC) { /* triggered also when INTERSECT_KEY */
				zend_sort((void *) lists[i], hash->nNumOfElements,
						sizeof(Bucket), zend_hash_bucket_swap, intersect_key_compare_func);
			}
		}
	}
...
static void php_array_diff(INTERNAL_FUNCTION_PARAMETERS, int behavior, int data_compare_type, int key_compare_type) /* {{{ */
{
	...
	for (i = 0; i < arr_argc; i++) {
		...
		if (hash->nNumOfElements > 1) {
			if (behavior == DIFF_NORMAL) {
				zend_sort((void *) lists[i], hash->nNumOfElements,
						sizeof(Bucket), zend_hash_bucket_swap, diff_data_compare_func);
			} else if (behavior & DIFF_ASSOC) { /* triggered also when DIFF_KEY */
				zend_sort((void *) lists[i], hash->nNumOfElements,
						sizeof(Bucket), zend_hash_bucket_swap, diff_key_compare_func);
			}
		}
	}
...
PHP_FUNCTION(array_multisort)
{
	...
	zend_sort(indirect, array_size, sizeof(Bucket *), array_bucket_p_sawp, php_multisort_compare);
